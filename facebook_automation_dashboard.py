...
OPENAI_API_KEY='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'
...
def user_login():
    ...
    if run_button_clicked:
        ...
        #using chat gpt to generate prompts for image
        clientopenai = OpenAI(api_key=OPENAI_API_KEY)
        response = clientopenai.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are a prompt engineering assistant ."},
            {"role": "user", "content": "Create a prompt for dall e to generate a well crafted image for"+ str(input_text)+"use keyword that help generate clear image and the text in the  image is correct and clearly visible and there should not be any deformed images"},
        ...
        if change_prompt == "y":
            ...
            st.text(" ctrl + enter to generate images")
